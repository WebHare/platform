...
/** Get the current state of the GTM datalayer in the test frame */
export function getCurrentDataLayer(): Record<string, unknown> {
  return getCurrentDataLayerVariables(getWin());
}

/** Get pxl log as generated by the current page */
export async function getPxlLogLines({ start = startTime, session = "" } = {}) {
  await invoke("mod::system/lib/testframework.whlib#FlushWebserverLogFiles"); //flush pxl logs. this requires HareScript inside the webserver, we can't do this through rpc("platform:frontendtests")
