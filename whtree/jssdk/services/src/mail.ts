...
class PreparedMail {
  ...
  async queue(): Promise<number[]> {
    ...
    const sender = await readRegistryKey<string>("system.services.smtp.mailfrom"); //FIXME read frmo witty's dom
...
// eslint-disable-next-line @typescript-eslint/no-empty-interface -- there will soon be options!
