...
function buildRichDescriptor(val: WrappedObjectFromWRDSetting) {
  return new RichFileDescriptor(val.size ? new WHDBBlobImplementation(val.id, val.size) : null, { ...decodeScanData(val.scandata), sourceFile: val.fs_object });
}
...
export function fieldsToJS(fields: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    } else if (value && typeof value === "object" && (value as WrappedObjectFromWRDSetting)["^$wrdtype"] === "fileimage")
      result[tagToJS(key)] = buildRichDescriptor(value as WrappedObjectFromWRDSetting);
...
export function repairResultValue(value: unknown) {
  ...
  if (value && typeof value === "object" && (value as WrappedObjectFromWRDSetting)["^$wrdtype"] === "fileimage")
