 ...
 * @param options - Optional statuscode and headers
 */
export function createJSONResponse<T = unknown>(status: HTTPStatusCode, jsonbody: T, options?: { headers?: Record<string, string>; format?: boolean }): WebResponse {
  const headers = { "content-type": "application/json", ...options?.headers };
  return createWebResponse(JSON.stringify(jsonbody, null, options?.format ? 2 : undefined), { status, headers });
}
