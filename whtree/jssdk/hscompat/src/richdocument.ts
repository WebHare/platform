...
class HSRTDImporter {
  ...
  async processInlineItem(child: Node, state: BlockItemStack, outlist: RTDInlineItems) {
    if (isElement(child)) {
      ...
      //Replace linefeeds with spaces as a linefeed in a JS string would be converted to a softbreak in Litty, but in HTML a linefeed is generally just whitespace (FIXME we have a litle <code> support - might matter there)
