...
test.runTests(
  [
    async function () {
      ...
      test.eq(450, test.getRoundedBoundingClientRect(currentimage).width);
      test.eq(600, test.getRoundedBoundingClientRect(currentimage).height);
      ...
      test.assert(test.canClick(currentimage));
      test.assert(!modalcontainer.classList.contains("wh-gallery-modal--firstslide"));
      test.eq(428, test.getRoundedBoundingClientRect(currentimage).width);
      test.eq(284, test.getRoundedBoundingClientRect(currentimage).height);
      ...
      await test.wait(() => test.qSA(".wh-gallery-modal__image--selected").length === 1);
      currentimage = test.qR(".wh-gallery-modal__image--selected");
      test.eq(600, test.getRoundedBoundingClientRect(currentimage).width);
      test.eq(450, test.getRoundedBoundingClientRect(currentimage).height);
      test.assert(modalcontainer.classList.contains("wh-gallery-modal--firstslide"));
      test.assert(!modalcontainer.classList.contains("wh-gallery-modal--lastslide"));
