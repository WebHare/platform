...
import type { WRDAttributeTypeId, SelectionResultRow, IsRequired, WRDAttr, Combine, WRDTypeBaseSettings, WRDBaseAttributeTypeId } from "@webhare/wrd/src/types";
...
async function testNewAPI() {
  ...
  */
  ...
  const firstperson = await schema.insert("wrdPerson", { ...basePerson, wrdInitials: "F", wrdInfix: "van de", wrdLastName: "lastname", wrdContactEmail: "first@beta.webhare.net", testJson: { mixedCase: [1, "yes!"], big: 4200420042n, date: new Date("2025-01-21T14:35:00Z") }, testJsonRequired: { mixedCase: [1, "yes!"] }, wrdGender: "male" });
  ...
  const deletedperson = await schema.insert("wrdPerson", { ...basePerson, wrdFirstName: "deleted", wrdLastName: "lastname3", wrdContactEmail: "deleted@beta.webhare.net", testRecord: testrecorddata as TestRecordDataInterface, testJsonRequired: { mixedCase: [1, "yes!"] }, wrdLimitDate: new Date(), wrdGender: "other" });
  ...
  test.eq({
    testinstance: (instance: Instance | null) => instance?.whfsType === "http://www.webhare.net/xmlns/beta/embedblock1" && instance?.data.id === "TestInstance-1" && instance?.data.fsref === 16 && instance?.data.styletitle === "",
  ...
  test.eq({
    testinstance: (instance: Instance | null) => instance?.whfsType === "http://www.webhare.net/xmlns/beta/embedblock1" && instance?.data.id === "TestInstance-2" && instance?.data.fsref === 1 && instance?.data.styletitle === "Test style",
  }, (await schema.getFields("wrdPerson", newperson, ["testinstance"])));
...
async function testImportMode() {
  ...
  await link.createAttribute("date", { attributeType: "plainDate", isRequired: true });
  await link.createAttribute("datetime", { attributeType: "instant", isRequired: true });
  await link.createAttribute("time", { attributeType: "plainTime", isRequired: true });
  ...
  await link.createAttribute("richDocument", { attributeType: "richTextDocument", isRequired: true });
