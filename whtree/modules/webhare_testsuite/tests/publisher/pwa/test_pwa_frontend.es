...
test.registerTests(
  ...
  , async function()
    {
      ...
      let swlog = await pwatests.getSWLog();
  ...
  , async function()
    {
      let clock = test.qS("#pwa-published-at").textContent;
  ...
  , async function()
    {
      ...
      let textfile = await test.getWin().fetch(test.qS("#mytextfilelink").dataset.textfilelink);
