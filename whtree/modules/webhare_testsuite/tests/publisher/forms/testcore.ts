...
test.runTests(
  [
    ...
    async function () {
      ...
      //submit it
      const prepareSubmitEvent = new Promise(resolve => formnode.addEventListener("wh:form-preparesubmit", evt => {
        test.eq(42, evt.detail.extrasubmit.proof);
        evt.detail.extrasubmit.preparesubmit = true;
        resolve(evt);
      }, { once: true }));
      const responseEvent = new Promise(resolve => formnode.addEventListener("wh:form-response", resolve, { once: true }));
      const submittedEvent = new Promise(resolve => formnode.addEventListener("wh:form-submitted", resolve, { once: true }));
      ...
      test.eqPartial({
        result: {
          email: "pietje",
          extradata: { proof: 42, preparesubmit: true },
