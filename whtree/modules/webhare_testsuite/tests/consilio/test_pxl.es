    ...
    , test: async (doc, win) =>
      {
        ...
        let lines = await test.invoke("mod::webhare_testsuite/tests/consilio/data/pxltest.whlib#GetPxlLogFiles", pxlEvent, startTime.toISOString());
        ...
        lines = await test.invoke("mod::webhare_testsuite/tests/consilio/data/pxltest.whlib#GetPxlLogFiles", pxlEvent, startTime.toISOString());
        test.true(lines.length > 0); // 1 or 2 lines, depending on value of preview cookie
