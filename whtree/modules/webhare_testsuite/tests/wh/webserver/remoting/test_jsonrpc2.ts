...
async function testRPCCaller() {
  ...
  const request = {
    ...
    url: "",
    headers: {},
    body: Buffer.from(JSON.stringify({ id: 5, method: "validateEmail", params: ["nl", "pietje@webhare.net"] })),
    method: HTTPMethod.POST
  };

  let callres = await JSONAPICall(servicedef, request);
  test.eq(200, callres.status);
  test.eq(false, JSON.parse(callres.body.toString()).result);
  test.eq(null, JSON.parse(callres.body.toString()).error, "It must be null if there was no error.");

  request.body = Buffer.from(JSON.stringify({ id: 42, method: "noSuchAPI", params: [] }));
  callres = await JSONAPICall(servicedef, request);
  test.eq(404, callres.status);

  test.eq({ id: 42, error: { code: -32601, message: `Method 'noSuchAPI' not found` }, result: null }, JSON.parse(callres.body.toString()));

  request.body = Buffer.from(JSON.stringify({ id: 77, method: "serverCrash", params: [] }));
  callres = await JSONAPICall(servicedef, request);
  ...
  //test.eq({ id: 77, error: { code: -32000, message: `this is a server crash` }, result: null }, JSON.parse(callres.body.toString()));
  test.eq({ id: 77, error: { code: -32000, message: `Internal error` }, result: null }, JSON.parse(callres.body.toString()));
}
