...
test.runTests(
  [
    ...
    async function () {
      ...
      const oldx = ridiculous_replacement.getBoundingClientRect().left;
      ...
      const waitforchange = test.waitForEvent(test.qS('#alwaysopendown'), 'change');
    ...
    async function () {
      ...
      test.eq(true, Boolean(menu));
      const visibleitems = testhelpers.getSelectListVisibleItems();
    ...
    async function () {
      ...
      replacement.click();
      test.eq(false, Boolean(testhelpers.getOpenSelectList()), 'unable to open optgroup pulldown by clicking it');
      items.childNodes[5].click();

      test.eq('3b', optgroupsselect.value);
      test.eq(false, Boolean(testhelpers.getOpenSelectList()), 'unable to close optgroup pulldown by clicking an option');
    ...
    async function () {
      ...
      test.eq(true, Boolean(testhelpers.getOpenSelectList()), 'pulldown should not close');
      test.eq('', test.qS('#ridiculous').value, 'disabled option should not be selected');

      const waitforchange = test.waitForEvent(test.qS('#ridiculous'), 'dompack:-internal-refreshed');
    ...
    function () {
      ...
      const justenoughmenu = testhelpers.getOpenSelectList();
      ...
      const onepixelshortmenu = testhelpers.getOpenSelectList();
    ...
    async function () {
      ...
      const waitforchange = test.waitForEvent(test.qS('#alwaysopendown'), 'dompack:-internal-refreshed');
    ...
    async function () {
      ...
      test.eq(false, Boolean(testhelpers.getOpenSelectList()), 'menu should initially be invisible');
      test.click(alwaysopendown_replacement);
      test.eq(false, Boolean(testhelpers.getOpenSelectList()), 'menu should still be invisible - we disabled the control before we clicked it');
