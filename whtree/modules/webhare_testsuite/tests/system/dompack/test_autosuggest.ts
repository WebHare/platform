...
test.runTests(
  [
    ...
    async function () {
      ...
      await test.waitUIFree();
      test.eq(false, Boolean(testhelpers.getOpenSelectList()), "There should be no selectlist");
      ...
      test.fill('#casesensitivelist', 'A');
      await test.waitUIFree();
      test.eq(true, Boolean(testhelpers.getOpenSelectList()));
      items = testhelpers.getSelectListVisibleItems();
      test.eq(2, items.length);
      test.eq("Aap", items[0].textContent);
    ...
    async function () {
      ...
      test.eq(true, Boolean(testhelpers.getOpenSelectList()), "list should immediately open");
      const items = testhelpers.getSelectListVisibleItems();
      test.click(items[0]);
    },
