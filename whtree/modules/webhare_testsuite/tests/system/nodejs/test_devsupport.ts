...
async function testDevBridge() {
  ...
  test.eq("@mod-system/js/internal/generated/whdb/webhare", devbridge.getImportPath(backendConfig.installationroot + "modules/system/js/internal/generated/whdb/webhare.ts"));

  const test_platform_files = await devbridge.getGeneratedFiles({ module: "consilio" });
  test.eqProps({ importPath: '@mod-system/js/internal/generated/whdb/webhare' }, test_platform_files.find(_ => _.type == "whdb"));
