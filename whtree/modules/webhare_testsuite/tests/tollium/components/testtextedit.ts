...
test.runTests(
  [
    ...
    async function () {
      ...
      let counter = textedit_comp.querySelector(".t-textedit__counter");

      test.fill(textedit, "1");
      await test.wait("ui");
      test.focus(textedit);
      test.assert(counter.classList.contains("t-textedit__counter--haveminvalue"));
      ...
      textedit_comp = test.compByName("componentpanel");
      textedit = textedit_comp.querySelector("input");
      counter = textedit_comp.querySelector(".t-textedit__counter");
    ...
    async function () {
      ...
      test.eq('rgb(252, 248, 208)', getComputedStyle(test.compByName("componentpanel").querySelector(".t-textedit__field")).backgroundColor);
      test.eq("none", getComputedStyle(test.compByName("componentpanel").querySelector(".t-textedit__field")).backgroundImage);

      //required + disabled REMOVES the background color (but SETS the disabled pattern)
      test.fill(test.compByTitle("Enabled"), false);
      await test.wait("ui");
      test.eq('rgba(0, 0, 0, 0)', getComputedStyle(test.compByName("componentpanel").querySelector(".t-textedit__field")).backgroundColor);
      test.eq(/^url/, getComputedStyle(test.compByName("componentpanel").querySelector(".t-textedit__field")).backgroundImage);
    ...
    async function () {
      ...
      const selection = tt.comp("selection");
      ...
      test.eq(4, textedit.querySelector("input")?.selectionStart);
