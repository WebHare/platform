...
test.runTests(
  [
    ...
    async function () {
      ...
      const selectel2 = test.compByName('righttopcell').querySelector('select');
      const datecomp = test.compByName('date1').querySelector('input[type=date]');
    ...
    async function () {
      ...
      const line2_2a_label = test.compByName('line2_2a').closest('.line').querySelector('.label');
      ...
      const firstinlineblockholder = test.compByName('inlineblock_select1_cell');
