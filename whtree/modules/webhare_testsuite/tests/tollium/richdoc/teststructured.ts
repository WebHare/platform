...
test.runTests(
  [
    ...
    async function () {
      ...
      const rte = rtetest.getRTE(test.getWin(), 'structured');
      rte.qSA<HTMLImageElement>('img').forEach(img => img.parentNode!.removeChild(img));
    ...
    async function () {
      ...
      test.eq(1, rte.qSA("img").length);
      test.assert(!rte.qR("img").hasAttribute("width"), "images without explicit width/height settings shouldn't have width/height");
      test.assert(!rte.qR("img").hasAttribute("height"), "images without explicit width/height settings shouldn't have width/height");
...

    {
      ...
      test: async function () {
        ...
        const imgnode = rte.qSA<HTMLImageElement>('img')[0];
        test.eq(13, imgnode.height);
        test.eq("13", imgnode.getAttribute("height"));
        test.eq("A", imgnode.parentElement?.nodeName.toUpperCase());
        test.eq("http://b-lex.nl/", imgnode.parentElement?.getAttribute("href"));
    ...
    //reopen the properties to verify and unset 'overridedimensions'
    {
      ...
      test: async function () {
        ...
        test.eq("A", imgnode.parentElement?.nodeName.toUpperCase());
        test.eq("http://b-lex.nl/nieuws/", imgnode.parentElement?.getAttribute("href"));
    ...
    },
    {
      ...
      test: async function () {
        const rte = rtetest.getRTE(test.getWin(), 'structured');
        const anode = rte.qSA<HTMLAnchorElement>('a')[1];
...

    {
      ...
      test: async function () {
        ...
        const htmlnode = rtenode.querySelector(".wh-rtd-editor-htmlnode");
...

    {
      ...
      test: async function () {
        ...
        const imgs = test.qSA(rte.editnode, 'img');
    ...
    async function () {
      ...
      const rte = rtetest.getRTE(test.getWin(), 'structured');
      const selection = rte.getEditor().getSelectionRange();
      const img = selection.querySelectorAll("img")[0] as HTMLImageElement;
      test.assert(img);
