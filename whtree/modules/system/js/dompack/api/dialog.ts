...
export class DialogBase
{
  ...
  async runModal()
  {
    ...
    if(!keyhandler)
      keyhandler = new KeyboardHandler(window, { "Escape": (evt: KeyboardEvent) => onEscape(evt)
...
export async function runMessageBox(question: string | HTMLElement | HTMLElement[], choices: DialogChoice[], options?: DialogOptions)
{
  ...
  if(dialog.buttonsnode) //this dialog has a separte node for the button area
    ...
    dialog.contentnode?.append(dompack.create("div", { className: dialogoptions ? dialogoptions.messageboxclassbase + "buttongroup" : ""
