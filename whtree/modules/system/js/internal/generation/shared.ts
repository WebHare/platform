...
interface WebHareVersionInfo {
  ...
  servername: string;
  modules: string[];
  /** Module versions, corresponding to modules array. Added as separate member (instead of replacing modules with an object or map) because WebHareVersionInfo appears in external APIs (peering) */
  modversions: string[];
...
export function getMyApplicabilityInfo({ unsafeEnv = false } = {}): WebHareVersionInfo {
  const mods = Object.entries(backendConfig.module);
  return {
    ...
    modules: mods.map(([name]) => name),
    modversions: mods.map(([, mod]) => mod.version || ''),
...
export function getApplicabilityError(webhareversioninfo: WebHareVersionInfo, restrictions: IfWebHare): string | null {
  ...
  for (const mod of restrictions.ifModules ?? []) {
    const [, modName, versionRange] = mod.match(/^([a-zA-Z0-9_]+)(.*)$/) || [];
    if (!modName || !isValidModuleName(modName))
      return `Invalid module name '${mod}' in ifModules restriction`;

    const modidx = webhareversioninfo.modules.indexOf(modName);
    if (modidx < 0)
      return `Module '${modName}' is not installed`;

    if (versionRange) {
      if (!webhareversioninfo.modversions)
        return `Module '${modName}' has no version and cannot be checked for version requirement '${versionRange}'`;

      const cleanrange = validRange(versionRange);
      if (!cleanrange)
        return `Module '${modName}' has an invalid version range '${versionRange}'`;
      if (!satisfies(webhareversioninfo.modversions[modidx], cleanrange))
        return `Module '${modName}' version '${webhareversioninfo.modversions[modidx]}' does not satisfy range '${cleanrange}'`;
    }
  }
