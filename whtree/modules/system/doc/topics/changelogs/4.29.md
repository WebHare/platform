...
- `DumpValue` now prints to stderr (outputid 2) by default (this interferes less with redirection and makes more sense for
  ...
  - If you have timed tasks using `DumpValue` they may now suddenly report to have failed, because the timed tasks executor
    considers any task printing to stderr to have failed (it does not watch the process exit code)
