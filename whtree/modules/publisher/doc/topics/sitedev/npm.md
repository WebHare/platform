...
## NPM modules with siteprofiles
__NOTE: This is still experimental functionality__

NPM modules can also be used to package HareScript and widgets in a reusable
way if you scope all namespaces used in the site profile.

### Inside the NPM module
Your module's `package.json` file should refer to a siteprofile which WebHare
should include when using your module:

```javascript
{
  "webhare_widget": { "siteprofile": "wh/mediagrid.siteprl.xml" }
}
```

This siteprofile should use `scope:` in namespace references as a prefix, eg
```xml
  <contenttype namespace="scope:mediafolder">
  </contenttype>
```

### Inside your webdesign

The WebHare module that uses your NPM module needs to use `<loadwidgetpackage>`
to load this NPM module (similar to `<addsiteprofile>`):
```xml
  <loadwidgetpackage module="wh-mediagrid" scopenamespace="http://mysite.nl/wh-mediagrid/" />
```

The above example would then be loaded as contenttype `https://mysite.nl/wh-mediagrid/mediafolder`
so your `<rtdtype>` would refer to the widget as:
```xml
  <widgets>
    <allowtype type="http://reelex.mst.nl/wh-mediagrid/mediagrid" />
  </widgets>
```

Your main JS file will likely still have to import some (S)CSS and JavaScript from
the NPM module to make everything work.
