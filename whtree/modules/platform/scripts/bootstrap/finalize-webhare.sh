...
rm -rf "$WEBHARE_HSBUILDCACHE" 2>/dev/null # Mostly useful on dev machines so the direct__ check later doesn't fail you. Ignore errors, usuall triggered by racing a running WebHare
(
