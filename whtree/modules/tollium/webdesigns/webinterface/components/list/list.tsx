...
export type FlatRowKey = string | number;
...
export default class ObjList extends ToddCompBase<ListAttributes> {
  ...
  _setSelection(rowkeys: FlatRowKey[]) {
    let changed = false;
  ...
  lookupRowByRowkey(rowkey: FlatRowKey) {
    for (let i = 0; i < this.flatrows.length; ++i)
  ...
  doNoLoopCheck(targetrow: FlatRow | null, sourcecomp: ToddCompBase, rowkeys: FlatRowKey[]) {
    if (sourcecomp !== this)
      return true;
