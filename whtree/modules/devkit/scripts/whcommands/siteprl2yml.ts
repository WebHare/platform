...
function importRTDType(ctxt: ImportContext, rt: CSPContentType): RTDType {
  ...
  return {
    ...
    ...rt.linkhandlers ? { linkHandlers: rt.linkhandlers.map(lh => lh.namespaceuri + '#' + lh.localname) } : {},
    ...rt.structure.tag_b ? { b: rt.structure.tag_b as "b" | "strong" } : {},
    ...rt.structure.tag_i ? { i: rt.structure.tag_i as "i" | "em" } : {},
  };
}
...
function importApplyRule(ctxt: ImportContext, ar: CSPApplyRule): ApplyRule {
  ...
  if (ar.baseproperties) {
    rule.baseProps = {};
    for (const binaryprop of ["description", "noarchive", "keywords", "noindex", "nofollow", "seotitle", "seotab", "striprtdextension", "title"] as const)
    ...
    if (ar.baseproperties.haslist.includes("SEOTABREQUIRERIGHT"))
      rule.baseProps.seoTabRequireRight = ar.baseproperties.seotabrequireright;
  }
