...
if [ -n "$ISMODULETEST" ]; then
  ...
  if ! $SUDO docker exec $CONTAINER wh assetpacks check "*:*"; then  #NOTE: wait for ALL assetpacks. might be nicer to wait only for dependencies, but we cant wait for just our own
    echo ""
