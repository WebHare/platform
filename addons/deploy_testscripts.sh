...
if ! ( curl --fail --user "info+buildbot@webhare.nl:$WHBUILD_BUILDBOTPASSWORD" -T $BASEDIR/testmodule.sh https://cms.webhare.dev/webdav/publisher/scripts/testmodule.sh &&
       curl --fail --user "info+buildbot@webhare.nl:$WHBUILD_BUILDBOTPASSWORD" -T $BASEDIR/docker-build/testcontainer.sh https://cms.webhare.dev/webdav/publisher/scripts/testcontainer.sh &&
