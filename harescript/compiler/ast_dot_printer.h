#ifndef blex_webhare_compiler_ast_dot_printer
#define blex_webhare_compiler_ast_dot_printer
...
class ASTThreadingInfo;

class AstDotPrinter : public AST::NodeVisitor<void, Empty>
{
    private:
        ...
        void BuildLink(AST::Node* from, AST::Node* to, signed id);

        void Visit(AST::Node* node);

        void OutputStartCode(Blex::Stream *stream, AST::Node *addr, const char *name);
        void OutputEndCode(Blex::Stream *stream);

        AstDotPrinter(AstDotPrinter const &) = delete;
        AstDotPrinter& operator=(AstDotPrinter const &) = delete;

    public:
        AstDotPrinter(CompilerContext &context);
        ~AstDotPrinter();

        void OutputASTNormal(AST::Node *node, Blex::Stream &output, TypeStorage const &tstorage);
